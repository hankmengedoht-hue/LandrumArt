  ...
  return `
    <article class="artwork-card${a.featured ? ' featured' : ''}" role="link" tabindex="0"
      ...
      <div class="card-info">
        ...
        <div class="card-meta">${[a.year].filter(Boolean).map(esc).join(' · ')}</div>
        ${a.dimensions ? `<div class="card-dims">${esc(a.dimensions)}</div>` : ''}
        ${(mp !== null || sold) ? `<div class="card-price">${sold ? 'Sold' : fmt(mp)}</div>` : ''}
      </div>
    </article>`;
}
    ...
    <div class="print-product" style="max-width:none;margin:0">
      ...
      <div class="print-controls" style="grid-template-columns:1fr">
        ...
        <div class="print-size-block">
          <div class="print-size-dims" style="visibility:hidden" aria-hidden="true">&nbsp;</div>
